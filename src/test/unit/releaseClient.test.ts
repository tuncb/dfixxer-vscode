...
describe("selectCompatibleReleaseAsset", () => {
  it("selects the asset from the pinned release tag", () => {
    const release: ReleaseRecord = {
      assets: [
        {
          contentType: "application/zip",
          downloadUrl: "https://example.invalid/windows-v0.11.1.zip",
          name: "dfixxer-windows-x86_64-v0.11.1.zip",
          size: 101,
        },
      ],
      draft: false,
    ...
    assert.deepEqual(
      ...
      {
        archiveType: "zip",
        assetName: "dfixxer-windows-x86_64-v0.11.1.zip",
        downloadUrl: "https://example.invalid/windows-v0.11.1.zip",
        releaseName: "Release v0.11.1",
        releaseTag: "v0.11.1",
        size: 101,
      },
    );
  ...
  it("maps linux and both macOS architectures to the documented archive names", () => {
        ...
        assets: [
          {
            ...
            downloadUrl: "https://example.invalid/linux-v0.11.1.tar.gz",
            name: "dfixxer-linux-x86_64-v0.11.1.tar.gz",
            size: 201,
          },
        ],
        draft: false,
    ...
    const macRelease = selectCompatibleReleaseAsset(
      {
        assets: [
          {
            contentType: "application/gzip",
            downloadUrl: "https://example.invalid/macos-v0.11.1.tar.gz",
            name: "dfixxer-macos-x86_64-v0.11.1.tar.gz",
            size: 202,
          },
        ],
        draft: false,
    ...
    assert.equal(linuxRelease.assetName, "dfixxer-linux-x86_64-v0.11.1.tar.gz");
    assert.equal(macRelease.archiveType, "tar.gz");
    assert.equal(macRelease.assetName, "dfixxer-macos-x86_64-v0.11.1.tar.gz");
