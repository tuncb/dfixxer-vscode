...
If `dfixxer.executablePath` is empty and no managed binary is installed yet, the extension prompts to install the pinned `dfixxer` release `v0.12.0` the first time you run:
...
- Downloads the pinned `v0.12.0` release to a temporary location, validates it, and only then replaces the managed executable
...
- The tag must match `package.json` exactly, for example `package.json` version `0.0.4` requires tag `v0.0.4`.
